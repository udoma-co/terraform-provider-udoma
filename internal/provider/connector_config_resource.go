...
func (c *ConnectorConfig) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	config, _, err := c.client.GetApi().CreateConnectorConfig(ctx).CreateOrUpdateConnectorConfigRequest(apiReq).Execute()
	if err != nil {
		resp.Diagnostics.AddError(
...
func (c *ConnectorConfig) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	apiReq := plan.toAPIRequest()
	config, _, err := c.client.GetApi().UpdateConnectorConfig(ctx, plan.ID.ValueString()).CreateOrUpdateConnectorConfigRequest(apiReq).Execute()
	if err != nil {
		resp.Diagnostics.AddError(
...
func (c *ConnectorConfigModel) fromApiResponse(config *api.ConnectorConfig) {
	c.ID = types.StringValue(config.ConnectorId)
	c.Name = types.StringValue(config.Name)
	c.Description = omittableStringValue(config.Description, c.Description)
	c.Enabled = types.BoolValue(config.Enabled)
	c.PingTime = types.StringValue(config.PingTime)
	c.SyncTime = types.StringValue(config.SyncTime)
}

func (c *ConnectorConfigModel) toAPIRequest() api.CreateOrUpdateConnectorConfigRequest {
	return api.CreateOrUpdateConnectorConfigRequest{
